    ...
    def run(self):
        ...
        for model in self.model_list:
            ...
            try:
                # 프로세스 시작
                if model == "NP":
                    self.log_signal.emit(f"[정보] NP 모델링 실행 중...")
                    self.process.start(settings.np_path, [stock_dir, json.dumps(self.stock_list)])
                elif model == "LGBM":
                    self.log_signal.emit(f"[정보] LGBM 모델링 실행 중...")
                    self.process.start(settings.darts_path, ['LGBM', stock_dir, json.dumps(self.stock_list)])
                elif model == "TFT":
                    self.log_signal.emit(f"[정보] TFT 모델링 실행 중...")
                    self.process.start(settings.darts_path, ['TFT', stock_dir, json.dumps(self.stock_list)])
                elif model == "XGB":
                    self.log_signal.emit(f"[정보] XGB 모델링 실행 중...")
                    self.process.start(settings.darts_path, ['XGB', stock_dir, json.dumps(self.stock_list)])
                elif model == "RE":
                    self.log_signal.emit(f"[정보] RE 모델링 실행 중...")
                    self.process.start(settings.darts_path, ['RE', stock_dir, json.dumps(self.stock_list)])
                ...
                # 예상치 못한 에러(파일 없음 등) 발생 시 종목 단위로 건너뛰기
                self.log_signal.emit(f"[에러] {model} 처리 중 중단됨: {str(e)}")
        self.log_signal.emit(f"[정보] 종가 예측 모델링 완료\n")
